...
import Matter, { Engine, Render, World, Bodies,Body,Mouse, MouseConstraint, Events, Constraint  } from 'matter-js';
...
const MatterJsDemo:React.FC<matterProps> = ({blocks}) => {
  ...
  const [textBoxPosition, setTextBoxPosition] = useState<{ x: number, y: number, rotation: number, offset: number, title: string} | null>(null);
  useEffect(() => {
    ...
    console.log('dirt xy', dirt_imageWidth, dirt_imageHeight, 300/dirt_imageHeight, width*2/dirt_imageWidth)
    ...
    const createDrop = () => {
      ...
      const block = Bodies.rectangle(window.innerWidth / 2, -300 + stringLength + (1*height)/numOfBoxes/2 + 10, width / 4, (1*height)/numOfBoxes, {
        friction: 1.0, // Set the friction value here
            ...
            render: {
                sprite: {
                  ...
                  yScale: ((1*height)/numOfBoxes) / imageHeight,
      ...
      for (let i = 0; i <= segmentCount; i++) {
        const segment = Bodies.rectangle(window.innerWidth / 2, -300+ i * segmentLength, 10, segmentLength, { isStatic: i === 0 });
      ...
      console.log('final coord', segments[segments.length - 1].position.y,)
      const finalConstraint = Constraint.create({
        bodyA: segments[segments.length - 1],
        bodyB: block,
        pointA: { x: 0, y: segmentLength / 2 },
        pointB: { x: 0, y:  -(1*height)/numOfBoxes / 2 },
        stiffness: 1,
      });
    ...
    render.canvas.addEventListener('mousedown', (event) => {
        ...
        for (let i = 1; i < bodies.length; i++) {
          ...
          if (Matter.Bounds.contains(body.bounds, mousePosition)) {
            ...
            const imageWidth = body.imageWidth;
            ...
            if (selectedBox.current){
                ...
                selectedBox.current.render.sprite.xScale = selectedBox.current.render.sprite.xScale * 1/1.2;
                selectedBox.current.render.sprite.yScale = selectedBox.current.render.sprite.yScale * 1/1.2;
            ...
            const targetOffset = (body.position.y - dis/2) - render.bounds.min.y;
            ...
            const topPercent = (((newY - totalOffset) -  (render.bounds.min.y - scrollY))) / window.innerHeight
            // console.log("total offset ", totalOffset);
            
            //console.log("relative" , body.position.x / window.innerWidth);

            // console.log('title ', names[-i + 12 + numOfBoxes], -i + 12 + numOfBoxes);
            setTextBoxPosition({ x: leftPercent, y: topPercent, rotation:rotation, offset: totalOffset, title: names[-i + 12 + numOfBoxes] });
    ...
    const handleScroll = () => {
      const scrollTop = window.scrollY;
        ...
        const parallax1Elements = document.querySelectorAll('#canvas');
          ...
          (element as HTMLElement).style.backgroundPositionY = `${scrollTop * -0.08}px`;
        });
  ...
  return <>
  <div ref={sceneRef} id = "canvas" className="fixed w-screen h-[calc(100vh+60px)] -z-10 "/>
  <div ref={overlayRef} className="fixed w-screen h-screen" />
  {textBoxPosition && <TextBox text="hiii" x={textBoxPosition.x} y={textBoxPosition.y} rotation={textBoxPosition.rotation} offset = {textBoxPosition.offset} title={textBoxPosition.title}/>}
  </>;
};
