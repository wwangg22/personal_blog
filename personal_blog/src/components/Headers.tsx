...
const Headers: React.FC<{
    ...
    // const user = userdata;
    // console.log('headers user', Object.keys(user).length, 'what?', Object.keys(user))
    ...
    useEffect(()=>{
        ...
        const handleScroll = () => {
            ...
            //         document.getElementById('titleicon')!.style.top = "-60px"
            //         document.getElementById('entire')!.style.top = "-150px"
            //     } else {
            //         document.getElementById("title")!.style.top = "24px";
            //         document.getElementById('titleicon')!.style.top = "24px"
            //         document.getElementById('entire')!.style.top = "0px"
            //     }
    ...
    return (
        ...
        <div id="entire" className='bg-[#FFFFFF80] wide:bg-[transparent] rounded-lg pt-6 grid grid-rows-headers fixed w-full duration-1000 z-10'>
            <div className='grid grid-cols-7 h-[75px]'>
                <div className="pl-[5vw] flex wide:pl-search wide:pr-searchright col-span-2 w-full">
                    {back && <Link className="w-full bg-transparent text-left text-2xl self-end" href = "/blog">Back</Link>}
                ...
                <div></div>
                <div></div>
                <h3 className="text-2xl text-right flex"></h3>
                <h3 className="text-2xl pl-4 flex">
                    <Link className="w-full bg-transparent text-left text-2xl self-end" href="/">Main Menu</Link>
                {
                    // (Object.keys(user).length && !isLoading &&
                    // <button className="self-end"onClick={clicked()}>
                    ...
                    // ||
                    // !isLoading && 
                    // (<Link className="self-end cursor-pointer w-full" href="/signin">sign in</Link>)
                }
                </h3>
            </div>
        ...
        {
            // dropdown && Object.keys(user).length ? <Profile /> : null
        }
        <div className='h-100'>
        </div>
        </>
